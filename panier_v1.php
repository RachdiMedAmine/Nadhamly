...
// Function to fetch data from the database for the user's panier
function fetchArticlesForUser($conn, $userId)
{
  // Sanitize and quote the user ID
  $userId = mysqli_real_escape_string($conn, $userId);

  // Query to check if id_equipe is not null in the panier table
  $query = "SELECT id_equipe FROM panier WHERE id_user = '$userId' AND id_equipe IS NOT NULL";
  $result = mysqli_query($conn, $query);

  // If there are rows with id_equipe not null, fetch data from the equipes table
  if (mysqli_num_rows($result) > 0) {
    $query = "SELECT e.image, e.libelle, e.prix 
                  FROM equipes AS e
                  INNER JOIN panier AS p ON e.id = p.id_equipe
                  WHERE p.id_user = '$userId'";
  } else {
    // If no rows with id_equipe not null, fetch data from the articles table
    $query = "SELECT a.image1, a.libelle, a.prix 
                  FROM articles AS a 
                  INNER JOIN panier AS p ON a.id = p.id
                  WHERE p.id_user = '$userId'";
  }
...
if (isset($_SESSION["utilisateur"])) {
  ...
  // Fetch articles from the database for the user's panier
  $articles = fetchArticlesForUser($conn, $userId);
} else {
  // If user is not logged in, show empty array
  $articles = [];
}
...
foreach ($articles as $article) {
  $initialSubtotal += $article['prix'];
}
$shipping = 50; // Assuming fixed shipping cost
$initialTotal = $initialSubtotal + $shipping;
      ...
      <tbody>
        <?php foreach ($articles as $article) { ?>
          <tr>
            ...
            <td>
              <input class="w-25 pl-1 quantity" value="1" type="number" min="1" data-price="<?php echo $article['prix']; ?>">
            </td>
            <td>
