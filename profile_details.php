...
$sql = "SELECT * FROM `articles` WHERE id_frs = '$id_uti'";
$res = mysqli_query($conn, $sql);

$sql2 = "SELECT * FROM `equipes` WHERE id_frs = '$id_uti'";
$res2 = mysqli_query($conn, $sql2);
$sql5 = "SELECT * FROM `candidatures` WHERE id_candidat = '$id_uti' and statut='acceptee'";
$res5 = mysqli_query($conn, $sql5);
$sql6 = "SELECT * FROM `utilisateur` WHERE id_user = $id_uti";
...
<head>
  ...
  <title><?php echo $row['nom'] . " " . $row['prenom']; ?></title>
...
<body class="vh 100">
    ...
    <div class="container-md mt-5" style="background-color: #fffbf7;padding:0;">
        ...
        <div class="col-auto text-start profile-username-pic mb-3 mb-md-0 ms-xxl-5">
          <div class="profile-pic" style="width: 150px; height: 150px;">
            <?php if (!empty($row['pdp'])) { ?>
              <img src="<?php echo $row['pdp']; ?>" alt="" width="150px" class="profile-picture" id="profile-image">
            <?php } else { ?>
          ...
          <div class="col-auto mt-2 d-none d-xxl-block" style="margin-left: 30px;">
            <!-- Username -->
            <h2 style="color:white;"><?php echo strtoupper($row['nom'] . " " . $row['prenom']); ?></h2>
            <!-- Role -->
            <h3><?php echo strtoupper($row['role']); ?></h3>
          </div>
        </div>
        ...
        <div class="col-auto text-xxl-start d-block d-xxl-none align-self-center">
          <div class="row">
            <div class="col-auto mt-3">
              <!-- Username -->
              <h2 style="color:#773a00;"><?php echo strtoupper($row['nom'] . " " . $row['prenom']); ?></h2>
              <!-- Role -->
              <h3><?php echo strtoupper($row['role']); ?></h3>
            </div>
          </div>
      ...
      <div class="container title-button">
        <div class="row justify-content-between align-items-center mt-2 mb-2">
          <div class="col-auto ">
            <h2 class="hidden" style="color:#773a00;">Articles</h2>
          </div>
      ...
      <div class="container">
        <div class="col-auto partie-container">
          <div class="row cont row-cols-1 row-cols-md-1 row-cols-lg-2 row-cols-xl-3 g-4 " style="margin-left:10px;margin-right:10px;">
            ...
            ?>
              <div class="col">
                <div class="card cont" <?php if ($row6['nombre'] == 1 && $row6['motif'] == 'Vente') { ?>onclick="window.location.href='details_art_vente_1.php?id=<?php echo $row6['id']; ?>';" <?php } elseif (($row6['nombre'] > 1 && $row6['motif'] == 'Vente')) {

                                                                                                                                                                                            ?> onclick="window.location.href='details_art_vente.php?id=<?php echo $row6['id']; ?>';" <?php
                                                                                                                                                                                          } elseif (($row6['nombre'] == 1 && $row6['motif'] == 'Location')) {
                                                                                            ?> onclick="window.location.href='details_art_location_1.php?id=<?php echo $row6['id']; ?>';" <?php
                                                                                                                                                                                          } else { ?> onclick="window.location.href='details_art_location.php?id=<?php echo $row6['id']; ?>';" <?php
                                                                                                                                                                                          } ?>>
                  <img src="images/upload/articles/<?php echo $row6['image1']; ?>" style="height:250px;" alt="Article Image">
                  <div class="card-body">
                    <p class="card-title"><?php echo $row6['libelle']; ?></p>
                    <p class="card-price"><?php echo $row6['prix'] . " DT"; ?></p>
                  </div>
                </div>
              </div>
            <?php
  ...
  <div class="container title-button">
    <div class="row justify-content-between align-items-center mt-2 mb-2">
      <div class="col-auto ">
        <h2 class="hidden" style="color:#773a00;">Equipes</h2>
      </div>
  ...
  <div class="container">
    <div class="col-auto partie-container">
      <div class="row cont row-cols-1 row-cols-md-1 row-cols-lg-2 row-cols-xl-3 g-4 " style="margin-left:10px;margin-right:10px;">
        <?php while ($row5 = mysqli_fetch_assoc($res2)) { ?>
          <div class="col">
            <div class="card cont" onclick="window.location.href='details_equipe.php?id=<?php echo $row5['id']; ?>';">
              <img src="images/upload/equipes/<?php echo $row5['image']; ?>" style="height:250px;" alt="Equipe Image">
              <div class="card-body">
                <p class="card-title"><?php echo $row5['libelle']; ?></p>
                <p class="card-price"><?php echo $row5['prix'] . " DT"; ?></p>
    ...
    <div class="col-auto partie-container">
      <div class="row cont row-cols-1 row-cols-md-1 row-cols-lg-2 row-cols-xl-3 g-4 " style="margin-left:10px;margin-right:10px;">
        ...
        while ($row6 = mysqli_fetch_assoc($res5)) {
          $sql6 = "SELECT * FROM `equipes` WHERE id='{$row6['id_equipe']}'";
          $res6 = mysqli_query($conn, $sql6);
          while ($row7 = mysqli_fetch_assoc($res6)) {
        ?>
            <div class="col">
              <div class="card cont" <?php if ($row6['id_frs'] == $id_user) { ?> onclick="window.location.href='details_equipe_perso.php?id=<?php echo $row6['id_equipe']; ?>';" <?php } else { ?> onclick="window.location.href='details_equipe.php?id=<?php echo $row6['id_equipe']; ?>';" <?php } ?>>
                <img src="images/upload/equipes/<?php echo $row7['image']; ?>" style="height:250px;" alt="Equipe Image">
                <div class="card-body">
                  <p class="card-title"><?php echo $row7['libelle']; ?></p>
                  <p class="card-price"><?php echo $row7['prix'] . " DT"; ?></p>
                </div>
              </div>
            </div>
        <?php }
